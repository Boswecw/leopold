<!-- src/lib/components/ObservationMap.svelte - COMPLETE FIXED VERSION -->
<script lang="ts">
  ...
  let audioPlayer: HTMLAudioElement | null = null; // FIXED: proper null type
  ...
  // FIXED: Handle all possible ObservationType values
  let layerControls: Record<string, boolean> = {
    visual: true,
    audio: true,
    'multi-modal': true,
    plant: true, // Added missing plant type
    verified: false
  };

  // FIXED: Simple initialization - no reactive statements to avoid circular dependencies
  let dateStart = activeFilters.date_range?.start || '';
  let dateEnd = activeFilters.date_range?.end || '';
  let minFrequency = activeFilters.audio_features?.min_frequency || 0;
  let maxFrequency = activeFilters.audio_features?.max_frequency || 20000;

  // FIXED: Initialize audio_features with all required properties
  if (!activeFilters.audio_features) {
    activeFilters.audio_features = {
      min_frequency: 0,
      max_frequency: 20000,
      call_types: [], // Added missing property
      audio_quality: [] // Added missing property
    };
  }

  if (!activeFilters.date_range) {
    activeFilters.date_range = { start: '', end: '' };
  }

  // FIXED: Simple update function - no circular dependencies
  function updateFilters() {
    // Update date range
    if (!activeFilters.date_range) {
      activeFilters.date_range = { start: '', end: '' };
    }
    activeFilters.date_range.start = dateStart;
    activeFilters.date_range.end = dateEnd;
    ...
    activeFilters.audio_features.min_frequency = minFrequency;
    activeFilters.audio_features.max_frequency = maxFrequency;
  ...
  function handleDateStartChange(event: Event) {
    ...
    dateStart = target.value;
    updateFilters();
  }

  function handleDateEndChange(event: Event) {
    const target = event.target as HTMLInputElement;
    dateEnd = target.value;
    updateFilters();
  }

  function handleMinFrequencyChange(event: Event) {
    const target = event.target as HTMLInputElement;
    minFrequency = parseInt(target.value) || 0;
    updateFilters();
  }

  function handleMaxFrequencyChange(event: Event) {
    const target = event.target as HTMLInputElement;
    maxFrequency = parseInt(target.value) || 20000;
    updateFilters();
  }

  // Keep only essential reactive statements (no circular dependencies)
  ...
  onMount(async () => {
    ...
    try {
      ...
      // FIXED: Import marker cluster plugin with proper error handling
      try {
  ...
  onDestroy(() => {
    if (audioPlayer) {
      audioPlayer.pause();
      audioPlayer = null; // FIXED: now properly typed
    }
    if (map) {
      map.remove();
    }
  ...
  async function initializeMap() {
    ...
    map = L.map(mapContainer, {
      center: centerLocation ? [centerLocation.latitude, centerLocation.longitude] : [39.8283, -98.5795],
      zoom: zoom,
      zoomControl: true,
    ...
    // FIXED: Initialize marker cluster group with proper error handling
    if (enableClustering && (L as any).markerClusterGroup) {
      try {
        ...
        console.warn('Marker clustering failed, falling back to regular markers:', clusterError);
        enableClustering = false;
      }
    }
  ...
  // FIXED: Proper type handling for layer controls
  function filterObservations(
    ...
    return obs.filter(observation => {
      // FIXED: Safe layer visibility filter
      const layerKey = observation.observation_type as string;
      ...
      if (filters.species && filters.species.length > 0) {
        if (!filters.species.includes(observation.species_name)) {
          return false;
        }
      }
      ...
      // FIXED: Audio frequency filter with proper null checks
      if (filters.audio_features && observation.audio_features && observation.audio_features.dominant_frequency) {
  ...
  function getObservationIcon(observation: Observation) {
    const confidence = observation.confidence_level || 0;
    ...
    switch (observation.observation_type) {
      ...
      case 'multi-modal':
        ...
        break;
      case 'plant':
        iconHtml = `<div class="marker-icon plant-marker" style="background-color: ${color}">🌱</div>`;
        break;
      default:
...
<div class="observation-map relative bg-white rounded-lg shadow-lg border border-primary-forest/10" style="height: {height}">
  <!-- Map Controls -->
  {#if showFilters}
      ...
      <div class="control-panel bg-white/95 backdrop-blur-sm rounded-lg shadow-lg border border-primary-forest/20 p-3">
        ...
        <div class="layer-toggles space-y-2">
          ...
          <label class="flex items-center gap-2 text-sm cursor-pointer">
            <input 
              type="checkbox" 
              bind:checked={layerControls.plant}
              on:change={() => toggleLayer('plant')}
              class="rounded border-gray-300 text-primary-forest focus:ring-primary-forest"
            />
            <span class="text-lg">🌱</span>
            <span>Plants ({observations.filter(o => o.observation_type === 'plant').length})</span>
          </label>
  ...
  {#if showFilterPanel}
    <div class="filter-panel absolute top-4 left-4 ml-20 z-[1000] bg-white/95 backdrop-blur-sm rounded-lg shadow-lg border border-primary-forest/20 p-4 w-80 max-h-96 overflow-y-auto">
      ...
      <!-- Date Range Filter - FIXED: Use event handlers -->
      <div class="filter-group mb-4">
        ...
        <div class="flex gap-2">
          <input
            type="date"
            value={dateStart}
            on:input={handleDateStartChange}
            class="flex-1 text-xs px-2 py-1 border border-gray-300 rounded focus:ring-1 focus:ring-primary-forest"
          />
          <input
            type="date"
            value={dateEnd}
            on:input={handleDateEndChange}
            class="flex-1 text-xs px-2 py-1 border border-gray-300 rounded focus:ring-1 focus:ring-primary-forest"
          />
        </div>
      </div>

      <!-- Audio Frequency Filter - FIXED: Use event handlers -->
      <div class="filter-group mb-4">
        ...
        <div class="flex gap-2 items-center">
          ...
          <input
            type="number"
            value={maxFrequency}
            on:input={handleMaxFrequencyChange}
            min="0"
            max="20000"
            class="w-20 text-xs px-2 py-1 border border-gray-300 rounded focus:ring-1 focus:ring-primary-forest"
          />
      ...
      <div class="filter-group mb-4">
        <label class="block text-sm font-medium text-gray-700 mb-2">
          Minimum Confidence: {activeFilters.confidence_min || 0}/5
        </label>
        <input
          type="range"
          min="0"
      ...
      <button
        on:click={() => {
          ...
          dateStart = activeFilters.date_range?.start || '';
          dateEnd = activeFilters.date_range?.end || '';
          minFrequency = activeFilters.audio_features?.min_frequency || 0;
          maxFrequency = activeFilters.audio_features?.max_frequency || 20000;
